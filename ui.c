...
static void draw_progress_locked()
{
    ...
    if (gProgressBarType != PROGRESSBAR_TYPE_NONE) {
        ...
        int dy = (2*gr_fb_height() + iconHeight - 2*height)/3;
...
static void draw_text_line(int row, const char* t, int align) {
    int col = 0;
    if (t[0] != '\0') {
        //int length = gr_measure(t);
        int length = strnlen(t, MENU_MAX_COLS) * CHAR_WIDTH;
        //if (length > MENU_MAX_COLS * CHAR_WIDTH)
        //    length = MENU_MAX_COLS * CHAR_WIDTH;
        switch(align)
        {
            case LEFT_ALIGN:
                col = 1;
        ...
        int w, h;
        gr_font_size(&w, &h);
        gr_text(col, (row+1)*CHAR_HEIGHT - (CHAR_HEIGHT-h)/2 - 1, t);
    }
}
...
static void draw_screen_locked(void)
{
    ...
    if (show_text) {
        ...
        if (show_menu) {
            ...
            ltime = time(NULL);
            ptm = localtime(&ltime);
            sprintf(buffer, "%02d:%02d", ptm->tm_hour, ptm->tm_min);
            draw_text_line(0, buffer, RIGHT_ALIGN);
            ...
            sprintf(buffer, "%d%%", batt_level);
            //draw_text_line(0, buffer, RIGHT_ALIGN);
            draw_text_line(1, buffer, RIGHT_ALIGN);
...
//    } else if (ev.type == EV_ABS && ev.code == 48 && ev.value == 0) {
    } else if (ev.type == 1 && ev.code == 330 && ev.value == 0) {
    	// ev.code = ABS_MT_TOUCH_MAJOR 0x30
		// finger lifted! lets run with this
#ifdef DEBUG_TOUCHSCREEN
    	ui_print("触摸坐标 X: %d, Y: %d\n", touch_x, touch_y);
    	FILE *f = NULL;
    	f = fopen("/tmp/test_touchscreen.log", "aw");
    	fprintf(f, "Touched x: %d, y: %d\n", touch_x, touch_y);
    	fclose(f);
#endif
		ev.type = EV_KEY; //touch panel support!!!
		int keywidth = gr_get_width(surface) / 4;
		if (touch_y > keyoffset_y &&
                touch_x > keyoffset_x && touch_x <= (keywidth * 4 + keyoffset_x)) {
			// they lifted in the touch panel region
			if (touch_x <= (keywidth + keyoffset_x)) {
				// down button
				ev.code = KEY_DOWN;
			} else if (touch_x <= (keywidth * 2 + keyoffset_x)) {
				// up button
				ev.code = KEY_UP;
			} else if (touch_x <= (keywidth * 3 + keyoffset_x)) {
				// back button
				ev.code = KEY_BACK;
			} else {
				// enter key
		...
		in_touch = 0;
		reset_gestures();
    } else if (ev.type == EV_ABS && ev.code == 53) {
    	// ev.code = ABS_MT_POSITION_X 0x35
        old_x = touch_x;
        touch_x = ev.value;
        if (old_x != 0)
        ...
        if (touch_y < keyoffset_y) {
            if (diff_x > 50 && diff_x > (diff_y >= 0 ? diff_y : -diff_y)) {
            	if (slide_left)
            		slide_left = 0;
            	else
            		slide_right = 1;
                reset_gestures();
            } else if (diff_x < -50 && -diff_x > (diff_y >= 0 ? diff_y : -diff_y)) {
            	if (slide_right)
            		slide_right = 0;
            	else
            		slide_left = 1;
    ...
    } else if (ev.type == EV_ABS && ev.code == 54) {
    	// ev.code = ABS_MT_POSITION_Y 0x36
        old_y = touch_y;
        touch_y = ev.value;
        if (old_y != 0)
        ...
        if (touch_y < keyoffset_y) {
            if (diff_y > 30 && diff_y > (diff_x >= 0 ? diff_x : -diff_x)) {
                ev.code = KEY_DOWN;
                ev.type = EV_KEY;
                reset_gestures();
            } else if (diff_y < -30 && -diff_y > (diff_x >= 0 ? diff_x : -diff_x)) {
                ev.code = KEY_UP;
                ev.type = EV_KEY;
                reset_gestures();
            }
...
int input_buttons()
{
    ...
    int keyoffset_x = (gr_fb_width() - gr_get_width(surface)) / 2;
    int keyoffset_y = gr_fb_height() - gr_get_height(surface);
    if (touch_x > keyoffset_x && touch_y > keyoffset_y) {
        if (touch_x <= (keywidth + keyoffset_x)) {
            //down button
            final_code = KEY_DOWN;
            start_draw = keyoffset_x + 1;
            end_draw = keywidth + keyoffset_x;
        } else if (touch_x <= (keywidth * 2 + keyoffset_x)) {
            //up button
            final_code = KEY_UP;
            start_draw = keywidth + keyoffset_x + 1;
            end_draw = keywidth * 2 + keyoffset_x;
        } else if (touch_x <= (keywidth * 3 + keyoffset_x)) {
            //back button
            final_code = KEY_BACK;
            start_draw = keywidth * 2 + keyoffset_x + 1;
            end_draw = keywidth * 3 + keyoffset_x;
        } else if (touch_x <= (keywidth * 4 + keyoffset_x)) {
            //enter key
            final_code = KEY_ENTER;
            start_draw = keywidth * 3 + keyoffset_x + 1;
            end_draw = keywidth * 4 + keyoffset_x;
        }
        pthread_mutex_lock(&gUpdateMutex);
        gr_color(0, 0, 0, 255);     // clear old touch points
        gr_fill(0, keyoffset_y-2, start_draw-1, keyoffset_y);
        gr_fill(end_draw+1, keyoffset_y-2, gr_fb_width(), keyoffset_y);
        gr_color(MENU_TEXT_COLOR);
        gr_fill(start_draw, keyoffset_y-2, end_draw, keyoffset_y);
        gr_flip();
        pthread_mutex_unlock(&gUpdateMutex);
    }

//    if (touch_y > keyoffset_y && touch_x > 0) {
//    }
