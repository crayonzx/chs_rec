...
try_update_binary(const char *path, ZipArchive *zip) {
    ...
    if (binary_entry == NULL) {
        ...
        if (update_script_entry != NULL) {
            ui_print("不再支持Amend脚本（update-script）\n");
            ui_print("Amend脚本由Google从Android 1.5开始不再赞成使用\n");
            ui_print("升级到基于姜饼系统CWM 3.0 recovery以后需要删除它\n");
            ui_print("制作可用的固件升级包时请转换到Edify脚本（updater-script和update-binary）\n");
